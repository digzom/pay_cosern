defmodule PayCosern.Controllers.CosernAccountController do
  import Plug.Conn
  ...
  def create(conn, %{"login" => login, "password" => password, "handle" => handle}) do
    user = Guardian.Plug.current_resource(conn)

    with true <- user.handle == handle,
         %User{id: _user_id} = user <- PayCosern.get_user_by_handle_with_accounts(handle),
